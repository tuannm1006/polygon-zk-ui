...
export type ContractInteractionChartProps = DefaultFcProps & {
  data: any[],
  onClick?: () => void,
};

export const ContractInteractionChart: React.FC<ContractInteractionChartProps> = ({
  className,
  data,
  onClick
}) => {
  ...
  return (
    <Panel className={clsx(className, 'relative')} onClick={onClick}>
      <SmallLineChart
        caption={chartOptions[DataKeys.total_degree].caption}
        value={total}
        data={data}
        dataLabel="index"
        dataKey="value"
        color={chartOptions[DataKeys.total_degree].color}
      />
